...
Definindo $r_T = \begin{bmatrix} r_1 \\ r_2/T \end{bmatrix}$,e
$$  $$
\begin{aligned}
\hat{V}(\hat{\beta}_T) &= s_T^2 (X_T'X_T)^{-1} \\
\hat{V}(r_T' \hat{\beta}_T) &= r_T' \hat{V}(\hat{\beta}_T) r_T  \\
&= s_T^2 r_T' (X_T'X_T)^{-1} r_T
\end{aligned}
$$

O teste de hipótese para $H_0: r_1 \alpha + r_2 \delta = r$ pode ser feito usando a estatística t:

$$
t = \frac{r_1 \hat{\alpha}_T + r_2 \hat{\delta}_T - r}{\sqrt{\hat{V}(r_T' \hat{\beta}_T)}}
$$

Sob a hipótese nula, esta estatística t segue uma distribuição t de Student com $T-k$ graus de liberdade, onde $k$ é o número de regressores na regressão.

### Testes de Hipóteses Múltiplas

Para testar várias hipóteses simultaneamente, usamos o teste F. Por exemplo, considere o caso de $m$ restrições:
$R\beta=r$, onde $R$ é uma matriz $m \times k$ e $r$ é um vetor $m \times 1$.  A estatística F é dada por:

$$
F = \frac{(R\hat{\beta} - r)'[R(X'X)^{-1}R']^{-1}(R\hat{\beta} - r)}{ms^2}
$$

Sob a hipótese nula, essa estatística segue uma distribuição F com $m$ e $T-k$ graus de liberdade.

## Modelos de Séries Temporais

Um modelo de séries temporais é usado para modelar dados que são coletados ao longo do tempo. Um dos modelos mais simples é o modelo auto-regressivo de ordem 1 (AR(1)).
...
Para garantir que a série temporal $y_t$ seja estacionária, o coeficiente $\phi_1$ deve satisfazer $|\phi_1| < 1$.

### Estimação do Modelo AR(1)

Os parâmetros $\phi_0$ e $\phi_1$ podem ser estimados usando o método de mínimos quadrados ordinários (OLS).  O estimador de OLS minimiza a soma dos quadrados dos erros:

$$
\min_{\phi_0, \phi_1} \sum_{t=2}^T (y_t - \phi_0 - \phi_1 y_{t-1})^2
$$

Este processo resulta em estimadores consistentes para os parâmetros.

### Testes para Modelos AR(1)

Uma vez que o modelo foi estimado, podemos fazer testes para avaliar a adequação do modelo. Podemos testar se $\phi_1=0$, que corresponde a verificar se o valor anterior de $y$ tem um impacto no valor atual.

Também podemos avaliar os resíduos do modelo para verificar se eles satisfazem as condições de ruído branco (média zero e variância constante).  Testes como o teste de Ljung-Box podem ser utilizados para verificar se existe autocorrelação nos resíduos.

### Modelos de Média Móvel (MA)

Outro tipo de modelo de séries temporais é o modelo de média móvel (MA). Um modelo MA(1) é definido como:
...
A estacionariedade do modelo MA(1) depende das condições do ruído branco. Este modelo é sempre estacionário.

### Modelos ARMA

Modelos ARMA combinam os componentes de modelos AR e MA. Um modelo ARMA(p,q) é dado por:

$$
y_t = \phi_0 + \phi_1 y_{t-1} + ... + \phi_p y_{t-p} + \epsilon_t + \theta_1 \epsilon_{t-1} + ... + \theta_q \epsilon_{t-q}
$$

A identificação da ordem (p,q) do modelo ARMA é fundamental para uma boa modelagem. Técnicas como a análise de funções de autocorrelação (ACF) e autocorrelação parcial (PACF) são usadas para auxiliar na identificação da ordem apropriada.
<!-- END -->
