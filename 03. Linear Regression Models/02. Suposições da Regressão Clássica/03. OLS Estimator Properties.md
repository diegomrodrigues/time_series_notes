...
Este resultado mostra que o estimador OLS *b* é não viesado. $\blacksquare$
...
VIII. E como $X'X(X'X)^{-1} = I$:
    $$Var(b) = \sigma^2 (X'X)^{-1}$$
Assim, a matriz de variância-covariância do estimador OLS é dada por  $\sigma^2(X'X)^{-1}$ $\blacksquare$
...
# Estimação de V chapéu (matriz de covariância dos erros)
